...
impl DecodeContext {
    ...
    fn charge_work(&mut self) -> Result<()> {
        let actual = self.work_units.checked_add(1).ok_or(Error::Overflow)?;
        self.check(DecodeLimit::WorkUnits, actual, self.limits.max_work_units)?;
...
impl<'de> Decoder<'de> {
    pub fn new(reader: &'de mut dyn Read) -> Self {
        ...
        Self {
            ...
            in_records: false,
            context: DecodeContext::new(limits).expect("default decode limits are valid"),
        }
    }
    ...
    pub(crate) fn request_with_limits(
        ...
        Ok(Self {
            ...
            path: VecDeque::with_capacity(PARSE_DEPTH),
            in_records: false,
            context: DecodeContext::new(limits)?,
        })
    }
    ...
    fn check_length(&self, kind: DecodeLimit, actual: usize) -> Result<()> {
        let limit = match kind {
            ...
            DecodeLimit::WorkUnits => self.context.limits.max_work_units,
        };
