...
impl<'a> Cursor<'a> {
    ...
    pub(crate) fn records(
        ...
        self.bytes_field(flexible, nullable)
    }

    pub(crate) fn validate_records(&mut self, range: &Range<usize>) -> Result<()> {
        if self.validate_record_sets {
            let _ = crate::record::borrowed::validate_with_budget(
